...
export default function App(){
  ...
  const CatEntry=({cats,title})=><div><div style={{fontSize:14,fontWeight:700,color:C.tx,marginBottom:6,marginTop:8}}>{title}</div><div style={sC}>
    <div style={{display:"grid",gridTemplateColumns:"1fr 60px 60px 50px 45px 1fr",gap:8,fontSize:10,color:C.mt,fontWeight:600,padding:"0 0 6px",borderBottom:"1px solid #eee"}}><span>Postavka</span><span>Plan</span><span>Izvedba</span><span>Razl.€</span><span>%</span><span>Komentar</span></div>
    {cats.map(cat=><React.Fragment key={cat.id}>
      <div style={{fontSize:12,fontWeight:700,color:C.tx,padding:"8px 0 2px",marginTop:4}}>{cat.nm}</div>
      {cat.subs.filter(sub=>subVis[sub.id]!==true).map(sub=>{const d=md.subs?.[sub.id]||{plan:0,actual:0,comment:""};const diff=d.plan-d.actual;const pct=d.plan?pc(d.actual,d.plan)+"%":"N/A";
        return<div key={sub.id} style={{display:"grid",gridTemplateColumns:"1fr 60px 60px 50px 45px 1fr",gap:8,fontSize:11,alignItems:"center",padding:"3px 0 3px 12px",borderBottom:`1px solid ${C.fn}`}}>
          <div style={{display:"flex",alignItems:"center",gap:4,fontSize:10}}><span>{sub.nm}</span><button type="button" onClick={()=>toggleSubVis(sub.id)} style={{background:"none",border:"none",cursor:"pointer",padding:0,fontSize:10,color:C.mt,flexShrink:0}}>👁</button></div>
  ...
  return<EB><div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",color:C.tx,minHeight:"100vh",background:C.bg}}>
    ...
    {vw==="dash"&&<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
        <div style={sM}><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:2}}>Prihodki</div><div style={{fontSize:22,fontWeight:700,color:C.gn}}>{fmt(tInc)}</div></div>
        <div style={sM}><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:2}}>Odhodki</div><div style={{fontSize:22,fontWeight:700,color:C.rd,marginBottom:2}}>{fmt(tAc)}</div><div style={{fontSize:9,color:"#999",lineHeight:1.4}}>F {fmt(tFx)}<br/>V {fmt(tVr)}{tUxt>0?<><br/>N {fmt(tUxt)}</>:""}</div></div>
        <div style={sM}><div style={{fontSize:9,color:C.mt,textTransform:"uppercase",marginBottom:2}}>Razlika</div><div style={{fontSize:22,fontWeight:700,color:tInc-tAc>=0?C.gn:C.rd}}>{tInc-tAc>=0?"+":""}{fmt(tInc-tAc)}</div></div>
        <div style={{...sM,position:"relative"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span style={{fontSize:9,color:C.mt,textTransform:"uppercase"}}>Varčevanje</span><span onClick={()=>setShowSavCfg(!showSavCfg)} style={{...sT("#dbeafe",C.bl),fontSize:8,cursor:"pointer"}}>⚙</span></div><div style={{fontSize:22,fontWeight:700,color:C.bl}}>{fmt(savVis.reduce((s,id)=>s+(md.subs?.[id]?.actual||0),0))}</div>{showSavCfg&&<div style={{position:"absolute",top:"100%",right:0,zIndex:20,background:"#fff",border:`1px solid ${C.bd}`,borderRadius:8,padding:10,minWidth:200,boxShadow:"0 4px 16px rgba(0,0,0,0.12)"}}>{CATS.find(c=>c.id==="savings_inv").subs.map(s=><label key={s.id} style={{display:"flex",alignItems:"center",gap:6,fontSize:11,padding:"2px 0",cursor:"pointer"}}><input type="checkbox" checked={savVis.includes(s.id)} onChange={e=>{if(e.target.checked)setSavVis(v=>[...v,s.id]);else setSavVis(v=>v.filter(x=>x!==s.id))}}/>{s.nm.substring(0,22)}</label>)}<button onClick={()=>setShowSavCfg(false)} style={{...sB(true),height:22,fontSize:10,marginTop:4,width:"100%"}}>OK</button></div>}</div>
      </div>

      {/* Plan by category - FIXED */}
      <div style={{fontSize:14,fontWeight:700,color:C.tx,marginBottom:4,marginTop:10}}>Fiksni stroški</div>
      ...
      <div style={{fontSize:14,fontWeight:700,color:C.tx,marginBottom:4,marginTop:8}}>Variabilni stroški</div>
    ...
    {vw==="annual"&&<div>
      ...
      {annualDetailMonth!==null&&<div style={{...sC,background:"#f0f7ff",border:`2px solid ${C.bl}`,marginBottom:12}}>
        <div style={{fontSize:13,fontWeight:700,color:C.tx,marginBottom:8}}>Mesečni pregled: {MF[annualDetailMonth]} {((md,annMonth)=>{const fxP=fxT(md,'plan');const fxA=fxT(md,'actual');const vrP=vrT(md,'plan');const vrA=vrT(md,'actual');const uxA=uxtT(md);return `(F: ${fN(fxA)}€ | V: ${fN(vrA)}€ | N: ${fN(uxA)}€ | Skupaj: ${fmt(fxA+vrA+uxA)})`})(yd[annualDetailMonth]||initM())}</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 60px 60px 50px 45px",gap:8,fontSize:10,color:C.mt,fontWeight:600,padding:"0 0 6px",borderBottom:"1px solid #eee"}}>
          <span>Postavka</span><span>Plan</span><span>Izvedba</span><span>Razl.€</span><span>%</span>
        </div>
        {visibleCats.map(cat=>{const pT=cT(yd[annualDetailMonth]||initM(),cat,'plan');const aT=cT(yd[annualDetailMonth]||initM(),cat,'actual');return<div key={cat.id}><div style={{fontSize:12,fontWeight:700,color:C.tx,padding:"8px 0 2px",marginTop:4}}>{cat.nm}</div>{cat.subs.filter(sub=>subVis[sub.id]!==true).map(sub=>{const d=yd[annualDetailMonth]?.subs?.[sub.id]||{plan:0,actual:0};const diff=d.plan-d.actual;const pct=d.plan?pc(d.actual,d.plan)+"%":"N/A";return<div key={sub.id} style={{display:"grid",gridTemplateColumns:"1fr 60px 60px 50px 45px",gap:8,fontSize:11,alignItems:"center",padding:"3px 0 3px 12px",borderBottom:`1px solid #f0f0f0`}}><span style={{fontSize:10}}>{sub.nm}</span><span style={{color:"#999",fontSize:10}}>{d.plan?fN(d.plan):"—"}</span><span style={{fontWeight:600}}>{d.actual?fN(d.actual):"—"}</span><span style={{fontSize:9,color:d.plan?(diff>=0?C.gn:C.rd):C.mt}}>{d.plan?(diff>=0?"+":"")+fN(diff):"—"}</span><span style={{fontSize:9,color:d.plan?(pc(d.actual,d.plan)>90?C.rd:C.gn):C.mt}}>{pct}</span></div>})}<div style={{display:"grid",gridTemplateColumns:"1fr 60px 60px 50px 45px",gap:8,fontSize:10,alignItems:"center",padding:"2px 0 2px 12px",borderTop:`1px solid ${C.bd}`,fontWeight:600,color:C.sb}}><span>{cat.nm} — Suma</span><span>{pT>0?fN(pT):"—"}</span><span>{aT>0?fN(aT):"—"}</span><span>{pT?(aT-pT>=0?"+":"")+fN(aT-pT):"—"}</span><span>{pT?pc(aT,pT)+"%":"—"}</span></div></div>})}
      </div>}

      {[{tp:"fixed",nm:"Fiksni stroški"},{tp:"var",nm:"Variabilni stroški"}].map(type=><div key={type.tp}><div style={{fontSize:14,fontWeight:700,color:C.tx,marginBottom:4,marginTop:type.tp==="var"?6:0}}>{type.nm}</div><div style={{...sC,overflowX:"auto"}}><table style={{width:"100%",fontSize:10,borderCollapse:"collapse"}}><thead><tr style={{color:C.mt}}><th style={{textAlign:"left",padding:"3px 6px",minWidth:"200px"}}>Postavka</th>{MS.map(m=><th key={m} style={{textAlign:"right",padding:"3px 3px",cursor:"pointer",fontWeight:600}} onClick={()=>{setMo(MS.indexOf(m));setVw("entry")}}>{m}</th>)}<th style={{textAlign:"right",padding:"3px 6px",fontWeight:700}}>Skupaj</th></tr></thead><tbody>{visibleCats.filter(c=>c.tp===type.tp).map(cat=><React.Fragment key={cat.id}><tr style={{background:"#f9fafb"}}><td colSpan={14} style={{padding:"6px 6px",fontSize:11,fontWeight:700,color:C.tx}}>{cat.nm}</td></tr>{cat.subs.filter(sub=>subVis[sub.id]!==true).map(sub=>{let tot=0;return<tr key={sub.id}><td style={{padding:"3px 6px 3px 16px",fontSize:9,color:"#666"}}>{sub.nm}</td>{Array.from({length:12},(_,i)=>{const mdata=yd[i]||initM();const v=mdata.subs?.[sub.id]?.actual||0;tot+=v;return<td key={i} style={{textAlign:"right",padding:"3px 3px",color:v>0?"#333":"#ccc"}}>{v>0?fN(v):"—"}</td>})}<td style={{textAlign:"right",padding:"3px 6px",fontWeight:500}}>{tot>0?fmt(tot):"—"}</td></tr>})}</React.Fragment>)}</tbody></table></div></div>)}

      <div style={{fontSize:14,fontWeight:700,color:C.tx,marginBottom:4,marginTop:6}}>Nepredvideni stroški</div>
      <div style={{...sC,overflowX:"auto"}}><table style={{width:"100%",fontSize:10,borderCollapse:"collapse"}}><thead><tr style={{color:C.mt}}><th style={{textAlign:"left",padding:"3px 6px"}}>Nepredvideni</th>{MS.map(m=><th key={m} style={{textAlign:"right",padding:"3px 3px",cursor:"pointer",fontWeight:600}} onClick={()=>{setMo(MS.indexOf(m));setVw("entry")}}>{m}</th>)}<th style={{textAlign:"right",padding:"3px 6px",fontWeight:700}}>Skupaj</th></tr></thead><tbody><tr style={{borderTop:`1px solid ${C.fn}`}}><td style={{padding:"3px 6px",fontWeight:500}}>Nepredvideni stroški</td>{Array.from({length:12},(_,i)=>{const mdata=yd[i]||initM();const v=uxtT(mdata);return<td key={i} style={{textAlign:"right",padding:"3px 3px",color:v>0?"#333":"#ccc"}}>{v>0?fN(v):"—"}</td>})}<td style={{textAlign:"right",padding:"3px 6px",fontWeight:700}}>{(() => {let t=0; for(let i=0;i<12;i++)t+=uxtT(yd[i]||initM()); return t>0?fmt(t):"—"})()}</td></tr></tbody></table></div>

      {compMode&&compYr&&<div style={{...sC,background:"#fefce8",border:"1px solid #fde68a",marginTop:10}}><div style={{fontSize:13,fontWeight:600,marginBottom:6}}>Primerjava {yr} vs {compYr}</div><table style={{width:"100%",fontSize:11,borderCollapse:"collapse"}}><thead><tr style={{color:C.mt}}><th style={{textAlign:"left",padding:4,minWidth:"200px"}}>Kategorija</th><th style={{textAlign:"right",padding:4}}>{yr}</th><th style={{textAlign:"right",padding:4}}>{compYr}</th><th style={{textAlign:"right",padding:4}}>±</th></tr></thead><tbody>{CATS.map(cat=>{let t1=0,t2=0;for(let m=0;m<12;m++){t1+=cT(yd[m]||initM(),cat,'actual');t2+=cT((data[compYr]||initY())[m]||initM(),cat,'actual')}const diff=t1-t2;return<tr key={cat.id} style={{borderTop:`1px solid ${C.fn}`}}><td style={{padding:4}}>{cat.nm}</td><td style={{textAlign:"right",padding:4}}>{fmt(t1)}</td><td style={{textAlign:"right",padding:4,color:C.mt}}>{fmt(t2)}</td><td style={{textAlign:"right",padding:4,fontWeight:600,color:diff>0?C.rd:diff<0?C.gn:C.mt}}>{diff>0?"+":""}{fmt(diff)}</td></tr>})}</tbody></table></div>}
